class Solution {
	private class UnionFind {
		...
		private UnionFind(int n){ //Link every number till max with itself(by assigning it to its index pos)
			parent = new int[n];
			for(int i=0;i<n;i++){
				parent[i] = i;
			}
		...
		private int getAbsoluteParent(int i){
			...
			parent[i]=getAbsoluteParent(parent[i]); //recusre till main parent ele is found
			return parent[i];
		}
	...
	public int largestComponentSize(int[] nums) {
		...
		UnionFind uf = new UnionFind(max+1); //Assign parent
